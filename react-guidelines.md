...
Comprehensive performance optimization guide for React applications (including Vite, Create React App, and other tooling), designed for AI agents and LLMs.  Contains 40+ rules across 8 categories, prioritized by impact from critical (eliminating waterfalls) to low (advanced patterns).
...
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   ...
   - 3.2 [Minimize Serialization at SSR Boundaries](#32)
   - 3.3 [Parallel Data Fetching with Component Composition](#33)
   - 3.4 [Per-Request Deduplication](#34)
...
```typescript
export async function handleRequest(request: Request) {
  const sessionPromise = authenticate(request)
...
Instead of awaiting data before returning JSX, use Suspense boundaries to show the wrapper UI faster while data loads. 
...
function Page() {
  ...
  useEffect(() => {
    fetchData().then(setData) // Blocks entire page
  }, [])
...
**Note:** This pattern works best with libraries that support Suspense for data fetching (React Query, SWR, Relay, etc.) or when using SSR frameworks with streaming support.
...
Popular icon and component libraries can have **up to 10,000 re-exports** in their entry file.  For many React packages, **it takes 200-800ms just to import them**, affecting both development speed and production bundle size.
...
**Alternative:  Vite plugin for automatic optimization**
...
export default defineConfig({
  plugins: [
    react(),
    // Use vite-plugin-optimize-barrel-exports
    {
      name: 'optimize-barrel-exports',
      transform(code, id) {
        // Transform barrel imports to direct imports
        // See: https://github.com/vitejs/vite/discussions/6730
      }
    }
  ]
})
...
module.exports = {
  resolve: {
    alias: {
      // Map barrel imports to direct paths
...
Libraries commonly affected:  `lucide-react`, `@mui/material`, `@mui/icons-material`, `@tabler/icons-react`, `react-icons`, `@headlessui/react`, `@radix-ui/react-*`, `lodash`, `ramda`, `date-fns`, `rxjs`.
...
**Alternative: Load after initial render**
...
**Vite-specific:  Explicit chunk naming**
...
For Node.js server-side rendering, use an LRU cache for data shared across requests.
...
### 3.2 Minimize Serialization at SSR Boundaries

When passing data from server to client (via initial state, props, etc.), only serialize fields that the client actually uses.
...
When using SSR, structure data fetching to run in parallel rather than sequentially.
...
export default function Page({ header, sidebarItems }) {
  return (
    <div>
      <div>{header}</div>
...
### 3.4 Per-Request Deduplication

Use a request-scoped cache for server-side request deduplication.  Authentication and database queries benefit most. 

**Implementation with AsyncLocalStorage (Node.js):**
...
app.get('/api/data', (req, res) => {
  withRequestCache(async () => {
    // Multiple calls to getCachedUser() within this request
    // will only execute the query once
...
Within a single request, multiple calls execute the query only once.
...
function UpdateButton() {
  ...
  return <button onClick={() => trigger({ name: 'New Name' })}>Update</button>
}
```
...
**Warning: global flag with stateful RegExp**
...
When hoisting RegExp with `/g` flag, remember it maintains state.  Reset `lastIndex` or recreate when needed.
...
Mutating props can cause React to not detect changes, leading to stale UI or unexpected behavior. 
...
2. [https://vitejs.dev](https://vitejs.dev)
3. [https://swr.vercel.app](https://swr.vercel.app)
4. [https://github.com/shuding/better-all](https://github.com/shuding/better-all)
5. [https://github.com/isaacs/node-lru-cache](https://github.com/isaacs/node-lru-cache)
6. [https://tanstack.com/query](https://tanstack.com/query)
